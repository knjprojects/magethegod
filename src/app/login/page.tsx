  ...
  const handleSubmit = async (e: any) => {
    ...
    if (auth.log) {
      ...
      try {
        ...
        const resData: any = await response.data;
        console.log(
          "getting sign up response in frontend :" + JSON.stringify(resData)
        );
    ...
    return (
      <div className="p-24 h-screen flex flex-col bg-black items-center">
        <div className="fixed top-0 right-0 flex flex-row m-4">
          <button
            className="justify-right text-right"
            onClick={() => auth.changeLog()}
          >
            {auth.log ? "Sign Up" : "Log In"}
          </button>
        </div>
        ...
        {auth.username != "guest" ? (
          <p className="text-center">
            Logged in with {JSON.stringify(loggedUser.username)}
          </p>
        ) : (
          <></>
        )}
